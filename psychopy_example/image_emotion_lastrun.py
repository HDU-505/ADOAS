...
startOfTheParadigm="start_paradigm"
endOfTheParadigm="end_paradigm"
startClosingEyes="start_closeeye"
endClosingEyes="end_closeeye"
startOpeningEyes="start_openeye"
endOpeningEyes="end_openeye"
startTask="start_task"
endTask="end_task"

oneTaskStart="start_onetask"
oneTaskEnd="end_onetask"
oneBlankStart="start_oneblank"
oneBlankEnd="end_oneblank"
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Desktop\\2026\\dataset\\psychopy\\image_emotion_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    marker = f"MARKER {startClosingEyes}"
    timestamp = local_clock()
    outlet.push_sample([marker], timestamp)
    ...
    marker = f"MARKER {endClosingEyes}"
    timestamp = local_clock()
    outlet.push_sample([marker], timestamp)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    ...
    marker = f"MARKER {startOpeningEyes}"
    timestamp = local_clock()
    outlet.push_sample([marker], timestamp)
    ...
    marker = f"MARKER {endOpeningEyes}"
    timestamp = local_clock()
    outlet.push_sample([marker], timestamp)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    ...
    for thisTrial in trials:
        ...
        for thisTrials_loop in trials_loop:
            ...
            import random
            jitter_duration = random.uniform(0.5, 1.2)
            ...
            while continueRoutine:
                ...
                if polygon.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > polygon.tStartRefresh + jitter_duration-frameTolerance:
                        # keep track of stop time/frame for later
        ...
        #        
                marker = f"MARKER {marker_to_send}"
                timestamp = local_clock()
                outlet.push_sample([marker], timestamp)
    ...
    marker = f"MARKER {endOfTheParadigm}"
    timestamp = local_clock()
    outlet.push_sample([marker], timestamp)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
